...
> **Objetivo:** Autenticação robusta com JWT, refresh token rotativo, OAuth2 Google e RBAC no backend do NestJS seguindo a Clean Architecture.

---

## Checklist de Tarefas

- [ ] **C-1:** Criar entidades de domínio (`Usuario`, `Perfil`, `RefreshToken`, `TokenAcao`)
- [ ] **C-2:** Criar repositórios abstratos (interfaces/ports) em `domain/repositories/`
- [ ] **C-3:** Implementar repositórios concretos Prisma em `infrastructure/repositories/`
- [ ] **C-4:** Implementar use case **Registrar Usuário**
- [ ] **C-5:** Implementar use case **Login** (credenciais + emissão JWT/RT)
- [ ] **C-6:** Implementar use case **Refresh Token** (rotação RTR)
- [ ] **C-7:** Implementar use case **Logout** (invalidação de refresh token)
- [ ] **C-8:** Implementar **Guard de Autenticação** (JWT) e **Autorização** (RBAC)
- [ ] **C-9:** Implementar **Rate Limiting** com Redis nas rotas de Auth
- [ ] **C-10:** Implementar use case **Recuperar Senha** (geração de token + simulação e-mail)
- [ ] **C-11:** Implementar use case **Confirmar E-mail**
- [ ] **C-12:** Implementar fluxo **OAuth2 Google**
- [ ] **C-13:** Implementar endpoints **Perfil do Usuário** (`GET` / `PATCH` `/users/me`)
...
* **Critério de Aceite:** O código compila sem importar qualquer dependência do framework NestJS ou do Prisma.

### C-3: Implementar repositórios concretos com Prisma
* **Tipo:** BE | **Dependência:** C-2, B-2
* **Entregável:** Classes concretas que implementam as interfaces em `infrastructure/repositories/` utilizando o Prisma Client.
* **Critério de Aceite:** Integração com o banco de dados é efetuada com sucesso nas rotas de teste.

### C-4: Implementar use case **Registrar Usuário**
* **Tipo:** BE | **Dependência:** C-3
* **Entregável:** Caso de uso em `application/use-cases/` realizando: hash de senha via `argon2`, associação do perfil `CLIENTE` e criação do token de ativação.
...
* **Entregável:** Caso de uso de login local emitindo Access Token JWT e Refresh Token gravado em cookie HttpOnly.
...
### C-8: Implementar **Guard de Autenticação** e **Guard de Autorização**
* **Tipo:** BE | **Dependência:** C-5
* **Entregável:** Classe `JwtAuthGuard` baseada em Passport JWT e `RolesGuard` verificando o perfil associado ao usuário.
* **Critério de Aceite:** Rotas decoradas com `@UseGuards(JwtAuthGuard, RolesGuard)` e `@Roles('ADMIN')` retornam `401 Unauthorized` se sem token, e `403 Forbidden` se o perfil do usuário logado não for ADMIN.

### C-9: Implementar **Rate Limiting** com Redis
* **Tipo:** BE | **Dependência:** C-5, A-6
* **Entregável:** Configuração do `ThrottlerModule` com `ThrottlerStorageRedisService` protegendo as rotas sob `/auth/*`.
...
* **Tipo:** BE | **Dependência:** C-3
* **Entregável:** Endpoints `/auth/forgot-password` e `/auth/reset-password` gerindo tokens temporários de ação.
...
* **Critério de Aceite:** Acesso à rota atualiza a flag `emailConfirmado` para `true` na tabela `usuarios`.

### C-12: Implementar fluxo **OAuth2 Google**
* **Tipo:** BE | **Dependência:** C-5
* **Entregável:** Rota `/auth/google` (redirecionamento) e `/auth/google/callback` utilizando passport-google-oauth20.
...
* **Entregável:** Rotas `GET /users/me` e `PATCH /users/me`.

...
- [ ] **E-1:** Criar entidade de domínio `Agendamento` com máquina de estados de status
- [ ] **E-2:** Criar repositório `Agendamento` (interface + Prisma)
- [ ] **E-3:** Implementar use case **Criar Agendamento** (validação de concorrência)
- [ ] **E-4:** Implementar use case **Listar Agendamentos do Usuário** (com filtros)
- [ ] **E-5:** Implementar use case **Cancelar Agendamento** (antecedência mínima)
- [ ] **E-6:** Implementar use cases admin: **Confirmar** e **Concluir** agendamento
- [ ] **E-7:** Implementar endpoint **Dashboard Admin** (agregador)
...
* **Entregável:** Interface `IAgendamentoRepository` na camada de domínio e a classe concreta `PrismaAgendamentoRepository` na camada de infraestrutura.
...
* **Entregável:** Caso de uso que valida a disponibilidade do slot selecionado e persiste o agendamento no banco de dados com status `PENDENTE`.
* **Critério de Aceite:**
...
* **Guia de Implementação:**
  * Para evitar problemas de concorrência (duas pessoas clicando no mesmo slot de forma simultânea), aplicar uma transação ou lock otimista no Prisma, ou usar o Redis com lock distribuído (`redlock`) antes da persistência no banco.
...
* **Guia de Implementação:**
  * Escrever queries eficientes com agregação do Prisma (`prisma.agendamento.groupBy` e `prisma.agendamento.aggregate`) para evitar gargalos de memória no servidor.

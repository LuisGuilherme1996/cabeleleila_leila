...
| **A-3** | Scaffold do app Node.js + Express em `apps/api/` com Clean Architecture (pastas `domain/`, `application/`, `infrastructure/`, `presentation/`) | BE    | A-2        | App Express rodando com endpoint health-check `GET /health`           | `curl localhost:3000/health` → `{ "status": "ok" }`   |
...
| **A-5** | Instalar e configurar componentes UI no projeto Angular + criar pacote `packages/ui/`                                                          | FE    | A-4        | Componentes UI disponíveis, botão de exemplo renderiza                | Componente de botão renderiza corretamente            |
...
| **A-8** | Configurar variáveis de ambiente (`.env.example`, `dotenv` no Express, `environment.ts` do Angular)                                            | INFRA | A-3, A-4   | `.env.example` documentado, Express lê variáveis via `dotenv`         | App sobe lendo `DATABASE_URL` e `REDIS_URL` do `.env` |

---

## Fase B — Banco de Dados & Migrations

> **Objetivo:** Ter todas as tabelas criadas, migrations versionadas via SQL puro e seeds básicos.

| ID      | Descrição                                                                                                                                | Tipo  | Depende de | Entregáveis                                              | Critério de Aceite                                          |
| ------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ----- | ---------- | -------------------------------------------------------- | ----------------------------------------------------------- |
| **B-1** | Configurar pool de conexão PostgreSQL com `pg` no `apps/api/` e criar helper de query                                                    | DB/BE | A-3, A-6   | Módulo `db/pool.ts` com pool configurado e helper tipado | Conexão ao PostgreSQL via pool funciona sem erro            |
| **B-2** | Criar migrations SQL do **Módulo IAM**: `usuarios`, `perfis`, `usuario_perfis`, `conexoes_oauth`, `sessoes_refresh_token`, `tokens_acao` | DB    | B-1        | Arquivos `.sql` em `migrations/` com DDL completo        | Execução das migrations cria tabelas e constraints no banco |
| **B-3** | Criar migrations SQL do **Módulo Catálogo**: `servicos`, `horarios_funcionamento`, `bloqueios_agenda`                                    | DB    | B-1        | Arquivos `.sql` com DDL e índices                        | Migration aplica sem erro                                   |
| **B-4** | Criar migration SQL do **Módulo Agendamentos**: `agendamentos` com FKs para `usuarios` e `servicos`                                      | DB    | B-2, B-3   | Arquivo `.sql` com enum de status e FK constraints       | Migration aplica, FK constraints validadas                  |
| **B-5** | Criar seed script com: perfis `ADMIN`/`CLIENTE`, usuário admin padrão, serviços de exemplo, horários de funcionamento padrão             | DB    | B-4        | `scripts/seed.ts` executável                             | `npx tsx scripts/seed.ts` popula dados corretamente         |
...
| **C-2**  | Criar **repositórios abstratos** (interfaces/ports) para cada entidade IAM em `domain/repositories/`                                 | BE   | C-1        | Interfaces `IUsuarioRepository`, `IPerfilRepository`, etc.           | Interfaces exportadas sem dependência de pg ou Express |
| **C-3**  | Implementar **repositórios concretos** com SQL puro (`pg`) em `infrastructure/repositories/`                                         | BE   | C-2, B-2   | Implementações SQL de cada repositório                               | Queries executam corretamente contra o DB              |
...
| **C-5**  | Implementar use case **Login** (validar credenciais, gerar JWT access token + refresh token em cookie HttpOnly)                      | BE   | C-4        | Use case + JWT signing configurado                                   | `POST /auth/login` retorna JWT e seta cookie           |
...
| **C-8**  | Implementar **Middleware de Autenticação** (JWT) e **Middleware de Autorização** (RBAC por perfil)                                   | BE   | C-5        | Middlewares `authMiddleware` e `roleMiddleware('ADMIN')`             | Rotas protegidas retornam 401/403 corretamente         |
| **C-9**  | Implementar **Rate Limiting** com Redis (`express-rate-limit` + `rate-limit-redis`) nas rotas `/auth/*`                              | BE   | C-5, A-6   | Rate limiter configurado com Redis store                             | Após N requests, retorna 429 Too Many Requests         |
...
| **C-13** | Implementar endpoint **Perfil do Usuário** (`GET /users/me`, `PATCH /users/me`)                                                      | BE   | C-8        | Router com rotas protegidas                                          | Usuário autenticado consulta e edita próprio perfil    |
...
| **D-2** | Criar repositórios (interfaces + implementações SQL com `pg`) para o módulo Catálogo                                               | BE   | D-1, B-3                | Repositórios funcionais                                         | Operações CRUD executam contra o DB                           |
| **D-3** | Implementar **CRUD de Serviços** (`GET`, `POST`, `PUT`, `PATCH /ativo`, `DELETE`) — rotas admin protegidas                         | BE   | D-2, C-8                | Router `servicosRouter`                                         | Admin cria/edita/desativa serviços; clientes só listam ativos |
| **D-4** | Implementar **CRUD de Horários de Funcionamento** — rotas admin protegidas                                                         | BE   | D-2, C-8                | Router `horariosFuncionamentoRouter`                            | Admin configura horários por dia da semana                    |
| **D-5** | Implementar **CRUD de Bloqueios de Agenda** (férias, feriados, manutenção) — rotas admin protegidas                                | BE   | D-2, C-8                | Router `bloqueiosAgendaRouter`                                  | Admin cria/remove bloqueios com motivo                        |
...
| **E-2** | Criar repositório (interface + SQL com `pg`) para `Agendamento`                                                                | BE   | E-1        | Repositório funcional com filtros por data, status e usuário | Queries executam corretamente                                      |
...
> [!WARNING]
> **Integrações críticas:** As tarefas D-6 (disponibilidade) e E-3 (criar agendamento) são as mais complexas pois cruzam dados de múltiplos módulos. Devem ser implementadas com atenção especial à concorrência e transações SQL.

...
- **RNF-02 (Backend Framework):** Node.js com express estruturado em **Clean Architecture** (camadas claras: `domain/`, `application/`, `infrastructure/`, `presentation/`), TypeScript, Node.js 22+.
- **RNF-03 (Frontend Framework):** Angular 21+, com gerência de estado reativa (Signals/NgRx component store) e **Tailwind CSS**.
- **RNF-04 (Design System):** fica em DESIGN.md, Angular UI primitives no pacote `packages/ui` para garantir componentes acessíveis e elegantes de acordo com as diretrizes de reutilização.
- **RNF-05 (Banco de Dados):** Banco de dados relacional **PostgreSQL** sem ORMS por enquanto nesse mvp, usando apenas **QueryBuilders**.
...
- **RNF-08 (Containers & DevOps):** Orquestração local do ambiente via `docker-compose.yml` contendo os serviços `postgres`, `redis`, `api` (Node.js com express) e `web` (Angular).
...
- [ ] O banco de dados PostgreSQL está versionado por migrations SQL e possui um script de seed completo para criar os schemas e tabelas.
